...
def _new_equation_step(name, state, action, params, structural, substeps, dt, length):
    """ETDRK4 on a fixed Fourier grid; no clipping or hidden fine labels.

    CH: u_t = -a*u_xxxx + b*(u^3-u)_xx + c*drive*sin(x).
    FHN: u_t = a*u_xx + u-u^3/3-v+c*drive*sin(x);
         v_t = b*(u+0.7-0.8*v).
    Structural error removes the cubic CH term or the FHN recovery feedback.
    """
    n=state.shape[-1]; k2=_wave_numbers(state,length).square()[None,None]
    a,b,c=(params[:,i:i+1,None] for i in range(3))
    phase=torch.arange(n,device=state.device,dtype=state.dtype)*(2*math.pi/n)
    forcing=c*action[...,None]*torch.sin(phase)[None,None]
    if name=='cahn_hilliard':
        linear=b*k2-a*k2.square()
        def nonlinear(v):
            y=torch.fft.irfft(v,n=n,dim=-1)
            reaction=torch.zeros_like(v) if structural else -b*k2*torch.fft.rfft(y.pow(3),dim=-1)
            f=torch.fft.rfft(forcing,dim=-1); f[...,0]=0
            return reaction+f
    else:
        linear=torch.cat((1-a*k2,(-0.8*b).expand(-1,1,k2.shape[-1])),dim=1)
        def nonlinear(v):
            y=torch.fft.irfft(v,n=n,dim=-1);u,w=y[:,0:1],y[:,1:2]
            feedback=torch.zeros_like(w) if structural else w
            return torch.fft.rfft(torch.cat((-u.pow(3)/3-feedback+forcing,b*(u+0.7)),dim=1),dim=-1)
    h=dt/substeps;z=(h*linear).to(torch.float64)
    E,E2=z.exp().to(state.dtype),(z/2).exp().to(state.dtype)
    Q=(h/2*_phi(z/2,1)).to(state.dtype)
    p1,p2,p3=(_phi(z,i) for i in (1,2,3))
    f1=(h*(p1-3*p2+4*p3)).to(state.dtype)
    f2=(h*(p2-2*p3)).to(state.dtype);f3=(h*(-p2+4*p3)).to(state.dtype)
    v=torch.fft.rfft(state,dim=-1)
    for _ in range(substeps):
        Nv=nonlinear(v);va=E2*v+Q*Nv;Na=nonlinear(va)
        vb=E2*v+Q*Na;Nb=nonlinear(vb);vc=E2*va+Q*(2*Nb-Nv);Nc=nonlinear(vc)
        v=E*v+f1*Nv+2*f2*(Na+Nb)+f3*Nc
    return torch.fft.irfft(v,n=n,dim=-1)
